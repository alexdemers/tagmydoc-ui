...
const ButtonRenderFunction = (props, ref) => {
    return _jsx("button", Object.assign({}, props, { ref: ref, className: resolveClassName(props) }), void 0);
};
export const resolveClassName = ({ disabled = false, circle = false, outline = false, intent = null, className = '', block = false, size = Size.md, shadow = false }) => {
    ...
    return `disabled:cursor-default hover:transition-colors focus:outline-none ${circle ? 'rounded-full focus:bg-gray-200' : 'focus:ring'} ${outline ? 'border' : ''} ease-in-out duration-100 whitespace-nowrap ${sizeClasses} ${colorClasses} ${className} items-center justify-center`;
};
